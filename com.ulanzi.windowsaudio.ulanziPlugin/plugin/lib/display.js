...
export function truncateLabel(label, limit = TITLE_LIMIT) {
    const text = label.trim() || "Audio";
    if (text.length <= limit)
        return text.toUpperCase();
    return `${text.slice(0, limit - 1).toUpperCase()}…`;
}
export function formatPercent(volume) {
    return `${toPercent(volume)}%`;
}
export function formatKeypadText(view) {
    if (view.error)
        return "!";
    if (view.waiting)
        return "...";
...
export function formatEncoderTitle(view) {
    if (view.error)
        return view.error;
    if (view.waiting)
        return "Waiting...";
...
export function volumeBar(percent, width = 10) {
    const filled = Math.round((Math.max(0, Math.min(100, percent)) / 100) * width);
    return `${"█".repeat(filled)}${"░".repeat(width - filled)}`;
}
export function buildVolumeIconSvg(view) {
